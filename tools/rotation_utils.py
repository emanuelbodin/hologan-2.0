...
def tf_voxel_meshgrid(height, width, depth, homogeneous = False):
    with tf.compat.v1.variable_scope('voxel_meshgrid'):
...
def tf_rotation_resampling(voxel_array, transformation_matrix, params, Scale_matrix = None, size=64, new_size=128):
    ...
    try:
        ...
        grid = tf.tile(tf.reshape(grid, (1, tf.compat.v1.to_int32(grid.get_shape()[0]) , tf.compat.v1.to_int32(grid.get_shape()[1]))), [batch_size, 1, 1])
